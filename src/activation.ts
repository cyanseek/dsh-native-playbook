...
export async function planNativeActivation(
  capability: string,
  options: NativeActivationOptions,
...
export async function activateNativeCapability(
  capability: string,
  options: NativeActivationOptions,
): Promise<ActivationResult> {
  const plan = await planNativeActivation(capability, options)
  if (!plan.allowed || !plan.recipe) return result(plan, 'withheld', false, false)
  ...
  if (existingSnapshot.exists) {
    throw new NativePlaybookError(
      'ACTIVATION_CONFLICT',
      `Activation state already exists for '${plan.recipe.id}' in profile '${options.profile}'.`,
    )
  }
  ...
  try {
    ...
    if (!verified.verified) {
      throw new NativePlaybookError(
        'ACTIVATION_FAILED',
        `DSH composed the profile, but '${capability}' did not become provider-ready.`,
      )
    }
    return result(
      plan,
...
export async function deactivateNativeCapability(
  capability: string,
  options: NativeActivationOptions,
): Promise<ActivationResult> {
  const plan = await planNativeActivation(capability, options)
  if (!plan.recipe) return result(plan, 'unchanged', false, false)
  const paths = activationPaths(options, plan.recipe)
  const state = await readOptional(paths.snapshot)
  if (!state.exists) return result(plan, 'unchanged', false, true)

  let snapshot: ActivationSnapshot
  try {
    snapshot = JSON.parse(state.content) as ActivationSnapshot
  } catch (error) {
  ...
  if (!current.exists || hash(current.content) !== snapshot.activatedHash) {
    throw new NativePlaybookError(
      'ACTIVATION_CONFLICT',
      'The profile patch changed after activation; refusing to overwrite the user\'s newer edits.',
    )
  }
  ...
  try {
    ...
    if (lifecycle?.status === 'ready') {
      throw new NativePlaybookError(
        'ROLLBACK_FAILED',
        `The restored profile still reports '${capability}' as active.`,
      )
    }
    await rm(paths.snapshot, { force: true })
    ...
    throw new NativePlaybookError(
      'ROLLBACK_FAILED',
      `Could not restore the pre-activation profile for '${capability}'.`,
      { cause: error },
    )
  }
}

export async function verifyNativeCapability(
  capability: string,
  options: NativeActivationOptions,
): Promise<{ profile: string; capability: string; verified: boolean; lifecycle?: CapabilityLifecycle }> {
...
export function resolveActiveDshProfile(argv: readonly string[]): string | undefined {
...
async function inspectProfile(options: NativeActivationOptions) {
...
function applyAllowlistedRecipe(source: string, recipe: ActivationRecipe): string {
  const document = parseDocument(source.trim() ? source : '[]\n')
  if (document.errors.length > 0 || !isSeq(document.contents)) {
    throw new NativePlaybookError(
      'ACTIVATION_CONFLICT',
      'The profile patch must be a valid top-level YAML sequence before it can be changed safely.',
    )
  }
  ...
  for (let index = 0; index < document.contents.items.length; index += 1) {
    ...
    if (value.id === 'session-query-sqlite') {
      if (JSON.stringify(value) !== JSON.stringify(desiredProvider)) {
        throw new NativePlaybookError(
          'ACTIVATION_CONFLICT',
          'The profile already customizes session-query-sqlite; preserving that user-owned configuration.',
        )
      }
      providerFound = true
    }
    ...
    for (const row of insert.items) {
      if (!isMap(row) || row.get('id') !== 'tool-session-query') continue
      const rowValue = row.toJSON()
      const expectedRow = (desiredToolPatch.insert as Array<Record<string, unknown>>)[0]
      if (JSON.stringify(rowValue) !== JSON.stringify(expectedRow)) {
        throw new NativePlaybookError(
          'ACTIVATION_CONFLICT',
          "The profile already owns a different 'tool-session-query' row.",
        )
      }
      toolFound = true
    }
  }
  if (!providerFound) document.add(desiredProvider)
  if (!toolFound) document.add(desiredToolPatch)
  const rendered = document.toString()
  return rendered.endsWith('\n') ? rendered : `${rendered}\n`
}

async function runDsh(options: NativeActivationOptions, args: string[]): Promise<string> {
  if (options.runDsh) return options.runDsh(args)
  try {
    ...
    throw new NativePlaybookError(
      ...
      `Could not compose DSH profile '${options.profile}'.`,
      { cause: error },
    )
  }
}

function activationPaths(options: NativeActivationOptions, recipe: ActivationRecipe) {
...
async function readOptional(filename: string): Promise<{ exists: boolean; content: string; mode?: number }> {
  try {
...
async function restoreSnapshot(filename: string, snapshot: ActivationSnapshot): Promise<void> {
...
function validateSnapshot(snapshot: ActivationSnapshot, recipe: ActivationRecipe, profile: string): void {
  if (
...
function result(
  plan: ActivationPlan,
  action: ActivationResult['action'],
  changed: boolean,
  verified: boolean,
