...
const RECIPE_FILES = ['session-search.yml'];
const ALLOWED_CAPABILITIES = new Set(['session_search', 'session_event_search']);
export async function listActivationRecipes() {
    return Promise.all(RECIPE_FILES.map(async (filename) => loadRecipe(join(RECIPE_DIRECTORY, filename))));
}
export async function findActivationRecipe(capability) {
    const recipes = await listActivationRecipes();
    return recipes.find((recipe) => recipe.capability === capability || recipe.provides.includes(capability));
}
...
function validateRecipe(candidate, filename) {
    if (!isRecord(candidate))
        invalid(filename, 'recipe must be an object');
    const recipe = candidate;
    if (recipe.schemaVersion !== 1)
        invalid(filename, 'schemaVersion must be 1');
    if (typeof recipe.id !== 'string' || !/^[a-z0-9-]+$/.test(recipe.id))
        invalid(filename, 'id is invalid');
    ...
    if (typeof recipe.description !== 'string' || recipe.description.length < 10)
        invalid(filename, 'description is missing');
    if (!isStringArray(recipe.upstreamEvidence) || recipe.upstreamEvidence.length === 0)
        invalid(filename, 'upstream evidence is missing');
    ...
    if (!isStringArray(recipe.compatibleDshVersions) || recipe.compatibleDshVersions.length === 0)
        invalid(filename, 'compatible versions are missing');
    ...
    if (!isStringArray(recipe.preconditions) || recipe.preconditions.length === 0)
        invalid(filename, 'preconditions are missing');
    if (recipe.risk !== 'low')
        invalid(filename, 'only low-risk recipes are allowed');
    ...
    if (!Array.isArray(recipe.patches) || recipe.patches.length !== 2)
        invalid(filename, 'patch list is incomplete');
    if (!isStringArray(recipe.verification) || recipe.verification.length === 0)
        invalid(filename, 'verification is missing');
    if (recipe.rollback !== 'restore-snapshot')
        invalid(filename, 'rollback must restore a snapshot');
    ...
    if (serialized !== expected)
        invalid(filename, 'patches do not match the reviewed allowlist');
    return candidate;
}
